  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await client.post('/schedules', {
        ...formData,
        quantity_plan: parseInt(formData.quantity_plan),
        quantity_fact: formData.quantity_fact ? parseInt(formData.quantity_fact) : null,
      });
      fetchSchedules();
      setShowForm(false);
      setFormData({
      ...
      {showForm && (
        <div className="card">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              <label className="form-label">Количество (план)</label>
              <input
                type="number"
                className="form-control"
                value={formData.quantity_plan}
                onChange={(e) => setFormData({ ...formData, quantity_plan: e.target.value })}
                required
              />
            </div>
            <div className="form-group">
          ...
          <tbody>
            {schedules.map(schedule => (
              <tr key={schedule.id}>
                ...
                <td>{schedule.quantity_plan} / {schedule.quantity_fact || 0}</td>
